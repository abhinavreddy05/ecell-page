      ...
      return this.each(function () {
        ...
        if ($(".loading-holder").length) {
          ...
          var preloadContainer = $(
            '<div class="loading-holder transparent" />'
          ).prependTo("body");
        ...
        var progressBar = $('<div class="progress-bar"></div>').appendTo(
            preloadContainer
          ),
          progressLoaded = $('<div class="progress-loaded"></div>').appendTo(
            progressBar
          );
        ...
        if (settings.debugMode) {
          ...
          console.log(
            ((new Date().getTime() - startTime) / 1000).toFixed(3) +
              ": scanning DOM for image elements..."
          );
        ...
        elementChildren.forEach(function (child) {
          if (
            ...
            (child.css("background-image") !== "none" &&
              child.css("background-image").indexOf("gradient") == -1)
          ) {
            if (
              !(
                settings.preloadSelector &&
                settings.showInContainer &&
                child === parent
              )
            ) {
              ...
              child
                .attr("data-bg", child.css("background-image"))
        ...
        if (settings.debugMode) {
          ...
          console.log(
            ((new Date().getTime() - startTime) / 1000).toFixed(3) +
              ": scanning DOM for image elements DONE"
          );
        }
          ...
          imageElements.forEach(function (element) {
            $.ajax({
              ...
              success: function (response, message, object) {
                element.fileSize = parseInt(
                  object.getResponseHeader("Content-Length")
                );
                totalPercentage += element.fileSize;

                if (settings.debugMode) {
                  console.log(
                    (element.fileSize / 1000).toFixed(2) +
                      " KB \t" +
                      (totalPercentage / 1000).toFixed(2) +
                      " KB total"
                  );
                }
                ...
                complete: function () {
                  ...
                  if (imagesLoaded === totalImages) {
                    if (settings.debugMode) {
                      ...
                      console.log(
                        ((new Date().getTime() - startTime) / 1000).toFixed(3) +
                          ": preloading image elements DONE"
                      );
                      console.groupEnd();
                    }
